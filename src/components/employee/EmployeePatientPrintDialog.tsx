...
interface EmployeePatientPrintDialogProps {
  open: boolean;
...
export const EmployeePatientPrintDialog: React.FC<EmployeePatientPrintDialogProps> = ({
  open,
  onOpenChange,
  patientId,
  patientName
  ...
  const handlePrint = async () => {
    ...
    try {
      // Navigate to employee print page with query parameters
      const printUrl = `/roles/employee/renseignement/patient/${patientId}/print?type=${selectedReport}&images=${includeImages}&history=${includeHistory}`;
            ...
            <RadioGroup value={selectedReport} onValueChange={(value) => setSelectedReport(value as ReportType)}>
              <div className="space-y-2">
                {reportOptions.map((option) => (
                  <div
                    key={option.id}
                    className={`flex items-start space-x-3 p-4 rounded-lg border-2 transition-colors cursor-pointer hover:bg-green-50 ${
