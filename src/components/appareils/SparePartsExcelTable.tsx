...
// Flatten spare parts to show one row per stock location
...
interface SparePartCreateData extends Partial<SparePart> {
  stockLocationId?: string;
  stockQuantity?: number;
}

interface SparePartsExcelTableProps {
  spareParts: SparePart[];
  stockLocations: Array<{ id: string; name: string }>;
  onSparePartUpdate: (sparePart: SparePart) => Promise<void>;
  onSparePartCreate: (sparePart: SparePartCreateData) => Promise<void>;
  onSparePartDelete: (id: string) => Promise<void>;
  onRefresh?: () => void;
}
...
export function SparePartsExcelTable({
  spareParts,
  stockLocations,
  onSparePartUpdate,
  onSparePartCreate,
  ...
  const [newRow, setNewRow] = useState<SparePartRow>({
    name: '',
    status: 'ACTIVE',
    minQuantity: 2,
    quantity: 0
  ...
  const [rowToDuplicate, setRowToDuplicate] = useState<SparePartRow | null>(null);
  ...
  const [rowToDelete, setRowToDelete] = useState<SparePartRow | null>(null);

  // Flatten spare parts to rows (one per stock location)
  const flattenSpareParts = (): SparePartRow[] => {
    const rows: SparePartRow[] = [];
  ...
  const allRows = flattenSpareParts();
  ...
  const filteredRows = allRows.filter(row => {
    ...
    if (searchTerm) {
      ...
      const matchesSearch =
        ...
        row.compatibleWith?.toLowerCase().includes(search);
  ...
  const getRowKey = (row: SparePartRow) => {
    return row.stockId || row.productId || 'new';
  };

  const handleEdit = (row: SparePartRow) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const cleanData = {
        ...
        partNumber: editedRow.partNumber,
        description: editedRow.description,
        compatibleWith: editedRow.compatibleWith,
      ...
      await onSparePartUpdate(cleanData as SparePart);
      ...
      toast({ title: "Succès", description: "Pièce de rechange mise à jour avec succès" });
    } catch (error) {
  ...
  const handleCancelNew = () => {
    ...
    setNewRow({ name: '', status: 'ACTIVE', minQuantity: 2, quantity: 0 });
  };
  ...
  const handleSaveNew = async () => {
    ...
    try {
      ...
      setIsAddingNew(false);
      setNewRow({ name: '', status: 'ACTIVE', minQuantity: 2, quantity: 0 });
      toast({ title: "Succès", description: "Pièce de rechange créée avec succès" });
    } catch (error) {
      toast({ title: "Erreur", description: "Erreur lors de la création", variant: "destructive" });
    }
  };

  const handleDelete = (row: SparePartRow) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await onSparePartDelete(rowToDelete.productId);
      setShowDeleteWarningDialog(false);
      setRowToDelete(null);
      toast({ title: "Succès", description: "Pièce de rechange supprimée avec succès (toutes les locations)" });
    } catch (error) {
  ...
  const handleDuplicateToLocation = (row: SparePartRow) => {
    if (!row.productId) return;
  ...
  const handleConfirmDuplicate = async () => {
    ...
    try {
      ...
      toast({ title: "Succès", description: "Pièce de rechange dupliquée vers un nouvel emplacement" });
  ...
  const updateEditedField = (field: keyof SparePartRow, value: any) => {
    if (editedRow) {
      setEditedRow({ ...editedRow, [field]: value });
    }
  };

  const updateNewField = (field: keyof SparePartRow, value: any) => {
  ...
  const renderCell = (row: SparePartRow, field: keyof SparePartRow, isEditing: boolean) => {
      ...
      <tr className="bg-blue-50 border-b border-blue-200">
        ...
        <td className="px-2 py-2">
          <Input
            ...
            onChange={(e) => updateNewField('partNumber', e.target.value)}
            className="h-8 text-xs w-32"
            placeholder="N° Pièce"
          />
        </td>
        ...
        <td className="px-2 py-2">
          <Input
            value={newRow.compatibleWith || ''}
            onChange={(e) => updateNewField('compatibleWith', e.target.value)}
            className="h-8 text-xs w-40"
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 bg-gray-50 p-4 rounded-lg border">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex-1 min-w-[200px]">
            <div className="relative">
              ...
              <Input
                placeholder="Rechercher par nom, marque, N° pièce..."
                value={searchTerm}
        ...
        <div className="text-xs text-muted-foreground">
          {filteredRows.length} pièce{filteredRows.length > 1 ? 's' : ''} de rechange trouvée{filteredRows.length > 1 ? 's' : ''}
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-100 border-b sticky top-0 z-10">
              <tr>
                ...
                <th className="px-2 py-3 text-left font-medium text-xs">N° Pièce</th>
                <th className="px-2 py-3 text-left font-medium text-xs">Description</th>
                <th className="px-2 py-3 text-left font-medium text-xs">Compatible</th>
              ...
              {paginatedRows.map((row) => {
                ...
                return (
                  <tr key={rowKey} className={`border-b ${rowClass}`}>
                    ...
                    <td className="px-2 py-2">{renderCell(row, 'partNumber', isEditing)}</td>
                    <td className="px-2 py-2">{renderCell(row, 'description', isEditing)}</td>
                    <td className="px-2 py-2">{renderCell(row, 'compatibleWith', isEditing)}</td>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">Pièce de rechange</label>
              <Input
      ...
      <Dialog open={showDeleteWarningDialog} onOpenChange={setShowDeleteWarningDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-red-600">⚠️ Attention - Suppression de pièce de rechange</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4">
              <p className="font-semibold text-yellow-800">
                Cette pièce de rechange existe dans plusieurs emplacements !
              </p>
              <p className="text-sm text-yellow-700 mt-2">
                Si vous supprimez cette pièce de rechange, <strong>tous les stocks dans toutes les locations seront définitivement supprimés</strong>.
              </p>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium">Pièce de rechange concernée :</label>
              <Input
              ...
              <div className="space-y-2">
                ...
                >
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">La pièce reste disponible dans les autres emplacements</div>
                  </div>
                </Button>
                ...
                >
                  <div>
                    <div className="font-medium">Supprimer complètement la pièce de rechange</div>
