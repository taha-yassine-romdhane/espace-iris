        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex items-center gap-2 px-4 py-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-green-900">{Object.keys(tunisiaRegions).filter(region => getRegionStats(region) > 0).length}</div>
                  <div className="text-xs text-green-600">Régions</div>
                </div>
              </div>

              <div className="flex items-center gap-2 px-4 py-3 bg-purple-50 rounded-lg border border-purple-200">
                <svg className="text-purple-600" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <rect x="4" y="4" width="16" height="12" rx="2"/>
                  <path d="M8 12h8"/>
                  <path d="M8 16h8"/>
                </svg>
                <div className="text-right">
                  <div className="text-lg font-bold text-purple-900">{filteredPatients.filter(p => p.hasDevices === true).length}</div>
                  <div className="text-xs text-purple-600">Avec équipements</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-sm font-medium text-blue-900 mb-1">Guide d'utilisation de la carte</h4>
          ...
          <div className="absolute bottom-4 left-4 bg-white rounded-lg shadow-lg border border-gray-200 p-4 z-[1000] max-w-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-3">
                <h5 className="text-xs font-semibold text-blue-900 mb-2">Statistiques rapides</h5>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="text-center">
                    <div className="font-bold text-blue-900">{filteredPatients.length}</div>
                    <div className="text-blue-600">Patients</div>
                  </div>
                  <div className="text-center">
                    <div className="font-bold text-green-900">{Object.keys(tunisiaRegions).filter(region => getRegionStats(region) > 0).length}</div>
                    <div className="text-green-600">Régions</div>
                  </div>
                </div>
              </div>
            </div>
