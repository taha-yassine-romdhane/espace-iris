...
const SalesTable = React.memo(({ onViewDetails, onEdit }: SalesTableProps) => {
  ...
  // Fetch sales data with employee filtering
  const { data: salesOperations, isLoading } = useQuery<Sale[]>({
    queryKey: ["employee-sales-operations", session?.user?.id],
    queryFn: async () => {
      try {
        // Filter sales by employee who created them
        const response = await fetch(`/api/sales?createdBy=${session?.user?.id}`);
        if (!response.ok) {
          throw new Error("Failed to fetch sales");
        }
        const data = await response.json();
        console.log("Employee sales data:", data);
        return data.sales || [];
      } catch (error) {
  ...
  const deleteSale = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["employee-sales-operations"] });
                      ...
                      <TableCell>
                        <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0 sm:space-x-2">
                          <Button 
                            ...
                            className="flex items-center justify-center gap-2 text-xs min-h-[36px] w-full sm:w-auto touch-manipulation hover:bg-green-50 border-green-200"
                          >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2">
              <button
                ...
                className="px-3 py-2 rounded-md text-sm font-medium text-gray-600 hover:text-green-600 disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation min-h-[40px]"
              >
                Précédent
              </button>
