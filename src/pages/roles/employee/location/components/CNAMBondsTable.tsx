...
export default function CNAMBondsTable({ rentalId, patientId, patientCnamId, deviceMonthlyRate, showGlobalView = false }: Props) {
  ...
  const [tempMonths, setTempMonths] = useState<number>(1);
  const [tempReminderDate, setTempReminderDate] = useState<Date | undefined>(undefined);
  const [editingBondForMonths, setEditingBondForMonths] = useState<'new' | string | null>(null);
          ...
          <Table className="min-w-max">
            ...
            <TableBody>
              ...
              {paginatedBonds.map((bond) => {
                ...
                return (
                  <TableRow key={bond.id} className={isEditing ? 'bg-green-50' : ''}>
                    {/* Numéros (BL + Dossier combined) - Sticky Left */}
                    <TableCell className={`sticky left-0 z-10 ${isEditing ? 'bg-green-50' : 'bg-white'}`}>
                      <div className="space-y-1">
                        <div className="flex items-center gap-1">
                          <span className="text-[10px] text-gray-500 whitespace-nowrap">BL (Auto):</span>
                          <Badge variant="outline" className="text-xs font-mono bg-green-50 text-green-700 border-green-200">
                      ...
                      <TableCell>
                        <div className="text-xs">
                          <div
                            ...
                            onClick={() => router.push(`/roles/employee/renseignement/patient/${bond.patient?.id}`)}
                          >
                    ...
                    {/* Actions - Sticky Right */}
                    <TableCell className={`sticky right-0 z-10 ${isEditing ? 'bg-green-50' : 'bg-white'}`}>
                      {isEditing ? (
                        <div className="flex gap-1">
                          <Button size="sm" variant="outline" onClick={handleSave} className="h-7 px-2 bg-white">
                            <Save className="h-3 w-3" />
          ...
          {selectedBondFinancials && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center bg-green-50 rounded p-2 border border-green-200">
                  <span className="text-sm text-green-700 font-medium">Montant Bon Total:</span>
